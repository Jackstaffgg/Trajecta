...
public class AiConclusionRateLimiter {
    ...
    public boolean isAllowed(Long userId) {
        ...
        return attempts < rateLimitProperties.getAiConclusion().getMaxAttempts();
    }
    ...
    public void onAttempt(Long userId) {
        ...
        if (attempts != null && attempts == 1L) {
            redisTemplate.expire(redisKey(userId), Duration.ofSeconds(rateLimitProperties.getAiConclusion().getWindowSeconds()));
        }
    }

    private String redisKey(Long userId) {
        return rateLimitProperties.getAiConclusion().getKeyPrefix() + userId;
    }
}
